...
def _file_path(code_agent: str, task_id: str) -> str:
    ...
    return (
        f"expriment_result/memory_agent_{code_agent}_feedback_fixed_guidance_4/"
        f"task_{task_id}_result.pkl"
    )
...
def sample_unique_task_index_pairs(
    task_ids: List[str], code_agent: str, n: int = 100, seed: int = 42
) -> List[Tuple[str, int]]:
    """
    ...
    if total_pairs < n:
        raise ValueError(f"Requested {n} pairs but only {total_pairs} unique pairs are available.")
...
def run_single(code_model, ablation_model, sampled_pair, huamn_feedback, args):
    """
    ...
    temp_output_dir = f"expriment_result/none_feedback_{code_model}"
    os.makedirs(temp_output_dir, exist_ok=True)

    result_file_path = os.path.join(input_dir, f"task_{task_id}_result.pkl")
    if not os.path.exists(result_file_path):
        print(
            f"Warning: Skipping pair ({task_id}, {turn_index}) because source file not found: {result_file_path}"
        )
        return
    ...
    print(code_agent.build_system_prompt())
    exit()
    ...
    feedback_dic = huamn_feedback
    feedback_info = None
    for guidance_level in range(4, 5):
        code_agent.set_history(data[turn_index]["agent_memory"])
        final_code = data[turn_index]["final_code"]
        DatasetManager.write_code_content(code_agent.work_dir, code_agent.task_id, final_code)

        # PROCESS-SAFE CHANGE: Create a unique file path for this specific run.
        output_file_path = os.path.join(
            temp_output_dir, f"result_{task_id}_{turn_index}_{guidance_level}.pkl"
        )
        ...
        feedback_content = "None feedback"
...
def run_exp(code_model, feedback_model, human_feedback, args, max_workers):
    """
    Parallel across *different* task_ids, but serialize items that share the same task_id.
    Shows progress with tqdm.
    """
    print(
        f"Starting ablation for code_model='{code_model}', ablation_model='{feedback_model}' with {max_workers} workers."
    )

    # 1) Bucket pairs by task_id
    buckets = defaultdict(deque)
    for pair, fb in human_feedback.items():
        task_id, _ = pair
        buckets[task_id].append((pair, fb))

    total = sum(len(q) for q in buckets.values())

    in_flight = {}  # future -> task_id
    active_task_ids = set()  # which task_ids currently have a running job

    def try_fill_slots(executor):
        for tid, q in buckets.items():
            ...
            if q:
                pair, fb = q.popleft()
                fut = executor.submit(run_single, code_model, feedback_model, pair, fb, args)
                in_flight[fut] = tid
                active_task_ids.add(tid)

    with ProcessPoolExecutor(max_workers=max_workers) as executor, tqdm(total=total) as pbar:
        try_fill_slots(executor)
...
def load_llmfeedback(result_dir, human_feedback):
    llm_feedback = {}
    for item in human_feedback.items():
        task_id, turn_num = item[0]
        resukt_file = os.path.join(result_dir, f"task_{task_id}_result.pkl")
        with open(resukt_file, "rb") as exp_result:
            data = pickle.load(exp_result)
        llm_feedback[item[0]] = data[turn_num]["feedback_interaction_history"][-1]["content"]
    return llm_feedback
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--yaml-location",
        type=str,
        default="config/config_gpt-5-bak/default_gpt4o_o1feedback_openai_level4_gpt4.1.yaml",
...
def load_human_feeback(huamn_feedback_path):
    with open(huamn_feedback_path, "rb") as human_feedback_file:
        data = pickle.load(human_feedback_file)
    return data
...
def main():
    ...
    huamn_feedback_path = "human_feedback.pkl"
    human_feedback = load_human_feeback(huamn_feedback_path)
    result_dir = f"expriment_result/memory_agent_{code_model}_feedback_fixed_guidance_4"
    llm_feedback = load_llmfeedback(result_dir, human_feedback)

    run_exp(code_model, feedback_model, llm_feedback, args, user_args.max_workers)
