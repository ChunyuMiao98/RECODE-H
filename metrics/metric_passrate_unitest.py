...
def get_pass_rates(pass_result):
    result = []
    for i in range(10):
        success = 0
        for single in pass_result:
            if i >= len(single):
                success += 1
            else:
                success += single[i]
        result.append(success/len(pass_result))
    return result

def get_model_passrates_turns(model, task_ids, result_dir=None):
    if result_dir is None:
        result_dir = 'expriment_result'
    level_results = []
    for guidance_level in [0,1,2,3,4]:
        ...
        for task_id in task_ids:
            ...
            for i in range(len(result_data)):
                exec_result = result_data[i]['execution_result']
                if eval_success(exec_result):
                    task_pass_status.append(1)
                    break
                else:
                    task_pass_status.append(0)
            pass_result.append(task_pass_status)
        pass_rates = get_pass_rates(pass_result)
        level_results.append(pass_rates)
    return level_results
...
if __name__ == '__main__':
    ...
    for key in task_id_map.keys():
        ...
        for model in models:
            ...
            # print(f"init {model}: {init_formatted}")
        # result = get_model_passrates_turns(model, task_ids)
        # print(result)
