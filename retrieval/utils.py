...
class FilePathBuilder:
    ...
    @staticmethod
    def repo_windows_path(cache_dir, repo, window_size, slice_size):
        repo_name = os.path.basename(repo)
        out_path = os.path.join(cache_dir, 'window', f'{repo_name}_ws{window_size}_slice{slice_size}.pkl')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path

    @staticmethod
    def repo_func_windows_path(cache_dir, repo, max_line_num, task_id):
        repo_name = os.path.basename(repo)
        out_path = os.path.join(cache_dir, 'window', f'{repo_name}_func_maxline{max_line_num}_{task_id}.pkl')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path
    ...
    @staticmethod
    def dense_embedding_vector_path(window_file, vectorizer):
        vector_path = window_file.replace('/window/', '/vector/')
        out_path = vector_path.replace('.pkl', f'_{vectorizer}.pth')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path
    ...
    @staticmethod
    def sfr_400M_vector_path(window_file):
        vector_path = window_file.replace('/window/', '/vector/')
        out_path = vector_path.replace('.pkl', '.sfr400m.pth')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path
    
    @staticmethod
    def sfr_2B_vector_path(window_file):
        vector_path = window_file.replace('/window/', '/vector/')
        out_path = vector_path.replace('.pkl', '.sfr2b.pth')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path

    @staticmethod
    def SFR_vector_path(window_file):
        out_path = window_file.replace('.pkl', '.pt')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path
...
class Tools:
    ...
    def iterate_repository(repo):
        pattern = os.path.join(f'{repo}', "**", "*.py")
        ...
        for fname in files:
            try:
                ...
                fpath_tuple = tuple(os.path.normpath(fname).split(os.sep))
